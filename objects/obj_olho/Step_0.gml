...
if (fire_timer <= 0) {
    ...
    // Só atira se o jogador existir e estiver distante
    if (instance_exists(obj_auro)) {
        var dist = point_distance(x, y, obj_auro.x, obj_auro.y);
        if (dist >= shoot_distance) {

            // Cria as 4 balas fixas (não mirando no jogador)
            var b;

            // Direita
            b = instance_create_layer(x, y, "Instances", obj_balainimiga);
            b.hspd = 6; b.vspd = 0;
            ...
            // Define limites da área jogável para todas as balas
